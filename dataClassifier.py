...
def enhancedFeatureExtractorDigit(datum):
    ...
    My enhanced feature for digit extraction is to 
    assign the numbers that each pixel type represents
    (#,+, or empty) to that pixel. The idea here is that 
    certain numbers will have edges in certain pixels most of the time.
    For example, 0 will have edges encircling the middle of the digit
    while the number 1 will not because that space is filled with normal
    black pixels. In this case, edge pixels are represented by '+' and 
    normal black pixels are represented by '#'.
    ##
    """
    a = datum.getPixels()
    features = util.Counter()
